...
:falcon512-shake-account-upgradeable: xref:/api/account.adoc#Falcon512ShakeAccountUpgradeable[Falcon512ShakeAccountUpgradeable]
:falcon512-shake-direct-account-upgradeable: xref:/api/account.adoc#Falcon512ShakeDirectAccountUpgradeable[Falcon512ShakeDirectAccountUpgradeable]
...
CAUTION: Before version 4.0.0-alpha.1, class hashes were computed using the `scarb --dev` profile.
...
| `{falcon512-shake-account-upgradeable}`
| `{Falcon512ShakeAccountUpgradeable-class-hash}`

| `{falcon512-shake-direct-account-upgradeable}`
...
IMPORTANT: Build Falcon account preset artifacts with
`scarb --release build -p openzeppelin_presets`. Their verification paths require compiler
inlining, so use the artifacts under `target/release` for declaration.
...
:presets_dir: link:https://github.com/OpenZeppelin/cairo-contracts/blob/release-v4.0.0-alpha.1/packages/presets/src[presets directory]
