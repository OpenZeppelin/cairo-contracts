...
func ERC721_Enumerable_tokenByIndex{
    ...
    with_attr error_message("ERC721_Enumerable_base: global index out of bounds"):
        assert is_lt = TRUE
    end
...
func ERC721_Enumerable_tokenOfOwnerByIndex{
    ...
    let (is_lt) = uint256_lt(index, len)
    with_attr error_message("ERC721_Enumerable_base: owner index out of bounds"):
        assert is_lt = TRUE
    end
...
func _add_token_to_all_tokens_enumeration{
    ...
    let (local new_supply: Uint256) = uint256_checked_add(supply, Uint256(1, 0))
...
func _remove_token_from_all_tokens_enumeration{
    ...
    let (local last_token_index: Uint256) = uint256_checked_sub_le(supply, Uint256(1, 0))
    ...
    let (local new_supply: Uint256) = uint256_checked_sub_le(supply, Uint256(1, 0))
    ERC721_Enumerable_all_tokens_len.write(new_supply)
    return ()
end
