...
async def account_factory():
  ...
  account = Account(starknet, signer, L1_ADDRESS)
...
async def test_initializer(account_factory):
  ...
  assert await account.call('get_public_key') == (signer.public_key,)
...
async def test_execute(account_factory):
  ...
  await account.send_transaction(initializable.contract_address, 'initialize', [])
