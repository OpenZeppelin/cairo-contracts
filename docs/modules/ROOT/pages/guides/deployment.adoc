...
A deployment is said to be counterfactual when the one paying for the deployment fees is the address of
the contract to be deployed. In other words, we need to send the funds to the address even when the
deploymenthasn’t happened.
