...
Creates an `amount` number of tokens and assigns them to `recipient`, by transferring it from the zero address.
...
Destroys an `amount` number of tokens from `account`, by transferring it to the zero address.
...
This internal function is equivalent to the external transfer, and can be used to implement automatic token fees, slashing mechanisms, etc.
