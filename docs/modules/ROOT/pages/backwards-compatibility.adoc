...
Bear in mind that while releasing versions, we treat minors as majors and patches as minors, in accordance with semantic versioning. This means that `v2.1.0` could be adding features to `v3.0.0`, while `v3.0.0` would be considered a breaking release.
