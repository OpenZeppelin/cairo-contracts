...
use openzeppelin_token::tests::mocks::erc20_mocks::{CamelERC20Mock, SnakeERC20Mock};
use openzeppelin_token::tests::mocks::erc20_mocks::{CamelERC20Panic, SnakeERC20Panic};
...
fn setup_snake() -> (DualCaseERC20, IERC20Dispatcher) {
    ...
    let target = test_utils::deploy(SnakeERC20Mock::TEST_CLASS_HASH, calldata);
...
fn setup_camel() -> (DualCaseERC20, IERC20CamelDispatcher) {
    ...
    calldata.append_serde(SUPPLY);
    calldata.append_serde(OWNER());
    let target = test_utils::deploy(CamelERC20Mock::TEST_CLASS_HASH, calldata);
...
fn setup_erc20_panic() -> (DualCaseERC20, DualCaseERC20) {
    let snake_target = test_utils::deploy(SnakeERC20Panic::TEST_CLASS_HASH, array![]);
    let camel_target = test_utils::deploy(CamelERC20Panic::TEST_CLASS_HASH, array![]);
    (
