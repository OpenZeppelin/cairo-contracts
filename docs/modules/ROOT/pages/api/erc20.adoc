...
NOTE: Any unfavorable discrepancy between <<IERC4626-convert_to_assets,convert_to_assets>> and `preview_mint` SHOULD be
...
NOTE: Any unfavorable discrepancy between <<IERC4626-convert_to_shares,convert_to_shares>> and `preview_withdraw`
SHOULD be considered slippage in share price or some other type of condition, meaning the
depositor will lose assets by depositing.
...
:mock-ex: xref:https://github.com/OpenZeppelin/cairo-contracts/tree/main/packages/test_common/src/mocks/erc4626.cairo[ERC4626FeesMock example]
...
The preview methods must return as close to the exact amount of shares or assets as possible if the actual (previewed) operation occurred in the same transaction (according to EIP-4626 spec).
...
Adjusts deposits within <<preview_mint,preview_mint>> to account for entry fees.
Entry fees should be transferred in the <<after_deposit,after_deposit>> hook.
...
Adjusts withdraws within <<preview_withdraw,preview_withdraw>> to account for exit fees.
Exit fees should be transferred in the <<before_withdraw,before_withdraw>> hook.
...
Adjusts withdraws within <<preview_redeem,preview_redeem>> to account for exit fees.
Exit fees should be transferred in the <<before_withdraw,before_withdraw>> hook.
...
NOTE: The EIP-4626 spec states that the `max_<OPERATION>` methods must take into account all
...
:mock-ex: xref:https://github.com/OpenZeppelin/cairo-contracts/tree/main/packages/test_common/src/mocks/erc4626.cairo[mock example]
...
NOTE: As per the EIP-4626 spec, this may panic _only_ if there's an overflow from an unreasonably large input.
...
Returns the maximum amount of the underlying asset that can be deposited into the Vault for the `receiver`, through a <<ERC4626Component-deposit,ERC4626::deposit>> call.
...
NOTE: This method must be inclusive of entry fees to be compliant with the EIP-4626 spec.
...
a <<ERC4626Component-mint,ERC4626::mint>> call.
...
Returns the maximum amount of the underlying asset that can be withdrawn from the owner
balance in the Vault, through a <<ERC4626Component-withdraw,ERC4626Component::withdraw>> call.
...
NOTE: This method must be inclusive of exit fees to be compliant with the EIP-4626 spec.
...
in the Vault, through a <<ERC4626Component-redeem,ERC4626Component::redeem>> call.
...
Internal logic for <<ERC4626Component-deposit, ERC4626::deposit>> and <<ERC4626Component-mint, ERC4626::mint>>.
...
Internal logic for <<ERC4626Component-withdraw, ERC4626::withdraw>> and <<ERC4626Component-redeem, ERC4626::redeem>>.
