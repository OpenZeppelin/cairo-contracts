...
In this particular example, the message struct will look like this:
...
Note that in practice is better to compute the type hash off-chain and hardcode it in the contract, since is a constant value.
...
const MESSAGE_TYPE_HASH = 0x120ae1bdaf7c1e48349da94bb8dad27351ca115d6605ce345aee02d68d99ec1;
...
=== 4. Implement the `SNIP12Metadata` trait to provide a dapp name and version.
...
a message hash needs to be generated, and this is an unecessary overhead.

[,javascript]

=== 5. Use the `OffchainMessageHashImpl` implementation to generate the hash.

The final step is to use the `OffchainMessageHashImpl` implementation to generate the hash of the message. The
implementation is already available as a utility. This is the full of a `get_hash` function:
...
fn get_hash(
    ...
    message.get_message_hash()
}
----

== Full Implementation

Finally, the full implementation of the `CustomERC20` contract will look like this:
