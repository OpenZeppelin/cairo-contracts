...
fn test_approve_from_owner() {
    ...
    assert_only_event_approval(OWNER(), SPENDER(), TOKEN_ID);
...
fn test_transfer_from_owner() {
    ...
    assert_only_event_transfer(owner, recipient, token_id);
...
fn test_transfer_from_to_owner() {
    ...
    assert_only_event_transfer(OWNER(), OWNER(), TOKEN_ID);
...
fn test_safe_transfer_from_to_account() {
    ...
    assert_only_event_transfer(owner, account, token_id);
...
fn test_safe_transfer_from_to_receiver() {
    ...
    assert_only_event_transfer(owner, receiver, token_id);
...
fn test__mint() {
    ...
    state._mint(recipient, TOKEN_ID);
    assert_only_event_transfer(ZERO(), recipient, token_id);
...
fn test__safe_mint_to_account() {
    ...
    assert_only_event_transfer(ZERO(), account, token_id);
...
fn test__burn() {
    ...
    state._burn(TOKEN_ID);
    assert_only_event_transfer(OWNER(), ZERO(), TOKEN_ID);
...
fn assert_only_event_approval(owner: ContractAddress, approved: ContractAddress, token_id: u256) {
    let event = utils::pop_log::<ERC721Component::Event>(ZERO()).unwrap();
...
fn assert_only_event_transfer(from: ContractAddress, to: ContractAddress, token_id: u256) {
