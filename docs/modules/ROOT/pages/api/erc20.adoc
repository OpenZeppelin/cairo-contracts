...
Calculates the entry fee for a deposit during <<ERC4626Component-preview_deposit,preview_deposit>>. The returned `Fee` is 
advisory only. It must be handled (e.g., transferred) manually in the <<ERC4626Component-after_deposit,after_deposit>> hook.
...
Calculates the entry fee for a mint during <<ERC4626Component-preview_mint,preview_mint>>. The returned `Fee` is 
advisory only. It must be handled (e.g., transferred) manually in the <<ERC4626Component-after_deposit,after_deposit>> hook.
...
Calculates the entry fee for a withdraw during <<ERC4626Component-preview_withdraw,preview_withdraw>>. The returned `Fee` is 
advisory only. It must be handled (e.g., transferred) manually in the <<ERC4626Component-before_withdraw,before_withdraw>> hook.
...
Calculates the entry fee for a redeem during <<ERC4626Component-preview_redeem,preview_redeem>>. The returned `Fee` is 
advisory only. It must be handled (e.g., transferred) manually in the <<ERC4626Component-before_withdraw,before_withdraw>> hook.
