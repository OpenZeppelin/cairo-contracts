...
[.contract-index#MultisigComponent-Internal-Impls]
.Internal Implementations
--
.InternalImpl
...
[.contract-index#MultisigComponent-Events]
.Events
--
...
Determines if the transaction with `id` is confirmed.
...
Checks if the transaction with `id` has been confirmed by the specified `signer`.

[.contract-item]
[[MultisigComponent-is_executed]]
==== `[.contract-item-name]#++is_executed++#++(self: @ContractState, id: TransactionID) → bool++` [.item-kind]#external#

Checks if the transaction with `id` has been executed.
...
Returns the current state of the transaction with `id`.
...
Returns the number of confirmations from registered signers for the transaction with `id`.
...
Returns the block number when the transaction with `id` was submitted.
