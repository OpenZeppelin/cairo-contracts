...
pub fn get_points() -> (Secp256k1Point, Secp256k1Point) {
...
pub fn assert_event_owner_added(contract: ContractAddress, public_key: EthPublicKey) {
    let event = test_utils::pop_log::<EthAccountComponent::Event>(contract).unwrap();
...
pub fn assert_only_event_owner_added(contract: ContractAddress, public_key: EthPublicKey) {
    assert_event_owner_added(contract, public_key);
    test_utils::assert_no_events_left(contract);
}

pub fn assert_event_owner_removed(contract: ContractAddress, public_key: EthPublicKey) {
    let event = test_utils::pop_log::<EthAccountComponent::Event>(contract).unwrap();
