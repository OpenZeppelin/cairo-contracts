...
use openzeppelin_token::tests::mocks::erc721_mocks::{CamelERC721Mock, SnakeERC721Mock};
use openzeppelin_token::tests::mocks::erc721_mocks::{CamelERC721PanicMock, SnakeERC721PanicMock};
...
fn setup_snake() -> (DualCaseERC721, IERC721Dispatcher) {
    ...
    let target = test_utils::deploy(SnakeERC721Mock::TEST_CLASS_HASH, calldata);
...
fn setup_camel() -> (DualCaseERC721, IERC721CamelOnlyDispatcher) {
    ...
    calldata.append_serde(TOKEN_ID);
    set_contract_address(OWNER());
    let target = test_utils::deploy(CamelERC721Mock::TEST_CLASS_HASH, calldata);
    (
...
fn setup_erc721_panic() -> (DualCaseERC721, DualCaseERC721) {
    let snake_target = test_utils::deploy(SnakeERC721PanicMock::TEST_CLASS_HASH, array![]);
    let camel_target = test_utils::deploy(CamelERC721PanicMock::TEST_CLASS_HASH, array![]);
    (
...
fn setup_receiver() -> ContractAddress {
    test_utils::deploy(DualCaseERC721ReceiverMock::TEST_CLASS_HASH, array![])
}
