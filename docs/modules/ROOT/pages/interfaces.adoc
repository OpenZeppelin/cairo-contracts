...
== Interface types
The library offers three types of traits:

=== Interface traits (`IERC20`)

These are associated with a predefined interface such a standard. This includes only the functions defined in the interface, and is the standard way to interact with a compliant contract.
...
They describe a contract's interface. This is useful to interface with a preset contract offered by this library, such as the ERC20 preset that includes non-standard functions like `increase_allowance`.
...
```javascript
let token = DualERC20 { contract_address: target };
