...
To deploy a Contracts for Cairo account variant, compile the contract and declare the class hash because it's likely not already declared on the network.
This also means that you'll need an account already deployed to the network.

Next, create the account JSON file with Starknet Foundry's `sncast` with the `account create` subcommand and include the `--class-hash` flag with the declared class hash.
