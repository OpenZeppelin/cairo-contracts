...
async def test_transfer_batch(erc1155_factory):
    ...
    balance_before = [0, 0, 0, 0]
    balance_before[0] = (await erc1155.balance_of(account.contract_address, token_id[0]).call()).result.res
    balance_before[1] = (await erc1155.balance_of(account.contract_address, token_id[1]).call()).result.res
    balance_before[2] = (await erc1155.balance_of(recipient, token_id[0]).call()).result.res
    balance_before[3] = (await erc1155.balance_of(recipient, token_id[1]).call()).result.res

    await signer.send_transaction(account, erc1155.contract_address, 'transfer_batch', [recipient, 2, 1, 2, 2, 100, 50])

    balance_after = [0, 0, 0, 0]
    balance_after[0] = (await erc1155.balance_of(account.contract_address, token_id[0]).call()).result.res
    balance_after[1] = (await erc1155.balance_of(account.contract_address, token_id[1]).call()).result.res
    balance_after[2] = (await erc1155.balance_of(recipient, token_id[0]).call()).result.res
    balance_after[3] = (await erc1155.balance_of(recipient, token_id[1]).call()).result.res

    assert [y - x for x, y in zip(balance_before,
                                  balance_after)] == [-100, -50, 100, 50]
...
async def test_transfert_batch_from(erc1155_factory):
    ...
    operations = [-5, -2, 5, 2]
    _, _, amount1, amount2 = operations
    ...
    await other.send_transaction(operator, erc1155.contract_address, 'safe_batch_transfert_from', [account.contract_address, operator.contract_address, 2, 1, 2, 2, amount1, amount2])

    expected = [x + y for x, y in zip(all_balances_1.result.res, operations)]
    ...
    assert all_balances_2.result.res == expected


@pytest.mark.asyncio
async def test_transfert_batch_from_error(erc1155_factory):
    _, erc1155, account, operator = erc1155_factory

    all_balances_1 = await erc1155.balance_of_batch([account.contract_address, account.contract_address, operator.contract_address, operator.contract_address], [1, 2, 1, 2]).call()
    operations = [- all_balances_1.result.res[0] -
                  1, -100, all_balances_1.result.res[0] + 1, 100]
    _, _, amount1, amount2 = operations
    # SETTING APPROVAL
    await signer.send_transaction(account, erc1155.contract_address, 'set_approval_for_all', [operator.contract_address, 1])
    try:
...
# TEST IF test_transfert_batch_from_error HAVE CHANGED A PART OF THE BATCH'S BALANCE --> test alone with previous test

# @pytest.mark.asyncio
# async def test_batch_error(erc1155_factory):
#     _, erc1155, account, operator = erc1155_factory
#     all_balances_1 = await erc1155.balance_of_batch([account.contract_address, account.contract_address, operator.contract_address, operator.contract_address], [1, 2, 1, 2]).call()
#     assert all_balances_1.result.res == 0
# //expect no operations made
