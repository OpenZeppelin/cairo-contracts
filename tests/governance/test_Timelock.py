...
from utils import Signer, assert_event_emitted, assert_revert, contract_path, get_block_timestamp, set_block_timestamp
...
async def timelock_factory():
    ...
    timelock = await starknet.deploy(
        ...
        constructor_calldata=[proposer_account.contract_address, executor_account.contract_address, DAY]
    )
...
async def test_constructor(timelock_factory):
    ...
    execution_info = await timelock.hasRole(PROPOSER_ROLE, proposer_account.contract_address).call()
    assert execution_info.result == (TRUE,)

    execution_info = await timelock.hasRole(CANCELLER_ROLE, proposer_account.contract_address).call()
    assert execution_info.result == (TRUE,)

    execution_info = await timelock.hasRole(EXECUTOR_ROLE, executor_account.contract_address).call()
    assert execution_info.result == (TRUE,)
...
async def test_schedule(timelock_factory):
    ...
    init_call = (initializable.contract_address, get_selector_from_name('initialize'), 0, 0)
    ...
    schedule_exec_info = await proposer_signer.send_transaction(proposer_account, timelock.contract_address, 'schedule', call_array)
...
async def test_revert_min_delay_too_low(timelock_factory):
    ...
    schedule_fun = proposer_signer.send_transaction(proposer_account, timelock.contract_address, 'schedule', call_array)
...
async def test_reschedule_fails(timelock_factory):
    ...
    await proposer_signer.send_transaction(proposer_account, timelock.contract_address, 'schedule', call_array)
    await assert_revert(proposer_signer.send_transaction(proposer_account, timelock.contract_address, 'schedule', call_array), "Timelock: operation already scheduled")
...
async def test_cancel(timelock_factory):
    ...
    await proposer_signer.send_transaction(proposer_account, timelock.contract_address, 'schedule', call_array)
    await proposer_signer.send_transaction(proposer_account, timelock.contract_address, 'cancel', [operation_hash])
...
async def test_execute(timelock_factory):
    ...
    await executor_signer.send_transaction(executor_account, timelock.contract_address, 'execute', exec_array)
