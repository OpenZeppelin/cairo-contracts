...
This function panics if `token_id` does not exist.
...
This function panics if `token_id` does not exist or if `to` is the current token owner.
...
This function panics if `owner` is also `operator`.
...
This function panics if `to` is the zero address or if the `token_id` already exists.
