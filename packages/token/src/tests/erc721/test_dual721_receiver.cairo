...
fn setup_snake() -> (DualCaseERC721Receiver, IERC721ReceiverDispatcher) {
    ...
    let target = test_utils::deploy(SnakeERC721ReceiverMock::TEST_CLASS_HASH, calldata);
    (
...
fn setup_camel() -> (DualCaseERC721Receiver, IERC721ReceiverCamelDispatcher) {
    let mut calldata = array![];
    let target = test_utils::deploy(CamelERC721ReceiverMock::TEST_CLASS_HASH, calldata);
    (
        DualCaseERC721Receiver { contract_address: target },
...
fn setup_erc721_receiver_panic() -> (DualCaseERC721Receiver, DualCaseERC721Receiver) {
    let snake_target = test_utils::deploy(SnakeERC721ReceiverPanicMock::TEST_CLASS_HASH, array![]);
    let camel_target = test_utils::deploy(CamelERC721ReceiverPanicMock::TEST_CLASS_HASH, array![]);
    (
