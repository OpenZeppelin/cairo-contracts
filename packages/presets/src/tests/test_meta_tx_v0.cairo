...
use snforge_std::{start_cheat_transaction_version_global, start_cheat_max_fee_global, start_cheat_chain_id_global, start_cheat_chain_id};
...
use openzeppelin_testing::constants::{ZERO, stark::KEY_PAIR};
use core::hash::{HashStateExTrait, HashStateTrait};
use core::pedersen::PedersenTrait;
use starknet::ContractAddress;
use openzeppelin_test_common::mocks::observer::{ObserverComponent, CallInfo, IObserverDispatcher, IObserverDispatcherTrait};
...
fn setup_dispatchers(exp_tx_hash: felt252, exp_chain_id: felt252) -> (MetaTransactionV0ABIDispatcher, ILegacyAccountDispatcher, ICounterDispatcher) {
...
fn test_meta_tx_v0_context_update() {
    ...
    let calls: Span<Call> = array![
        Call { to: COUNTER, selector: selector!("increase_by"), calldata: array![incr_1.into()].span() },
        Call { to: COUNTER, selector: selector!("increase_by"), calldata: array![incr_2.into()].span() },
        Call { to: COUNTER, selector: selector!("increase_by"), calldata: array![incr_3.into()].span() },
    ].span();
    ...
    // Setup
    let (meta_tx_dispatcher, account_dispatcher, counter_dispatcher) = setup_dispatchers(tx_hash, CHAIN_ID);
    println!("--------------------------------");
    ...
    meta_tx_dispatcher
        .execute_meta_tx_v0(
            account_dispatcher.contract_address,
            EXECUTE_SELECTOR,
            calldata,
            signature,
        )
...
fn compute_invoke_v0_tx_hash(contract_address: ContractAddress, selector: felt252, calldata: Span<felt252>) -> felt252 {
    let mut calldata_hash = PedersenTrait::new(0);
    for elem in calldata {
        calldata_hash = calldata_hash.update_with(*elem);
    }
    let calldata_hash = calldata_hash
        .update_with(calldata.len())
        .finalize();
    PedersenTrait::new(0)
        .update_with('invoke')
...
impl SpyHelpersImpl of SpyHelpers {
    fn assert_event_external_call(
        ...
        let expected = ObserverComponent::Event::ExternalCall(CallInfo { caller, tx_version, chain_id, tx_hash });
