...
:RoleGrantedWithDelay: xref:#IAccessControl-RoleGrantedWithDelay[RoleGrantedWithDelay]
...
* xref:IAccessControl-RoleGrantedWithDelay[`++RoleGrantedWithDelay(role, account, sender, delay)++`]
...
=== `++IAccessControlWithDelay++` link:https://github.com/OpenZeppelin/cairo-contracts/blob/release-v1.0.0/packages/access/src/accesscontrol/interface.cairo[{github-icon},role=heading-link]

:get_role_status: xref:#IAccessControlWithDelay-get_role_status[get_role_status]
...
.Events
--
* xref:IAccessControlWithDelay-RoleAdminChanged[`++RoleAdminChanged(role, previous_admin_role, new_admin_role)++`]
...
==== `[.contract-item-name]#++grant_role_with_delay++#++(role: felt252, account: ContractAddress, delay: u64) → felt252++` [.item-kind]#external#
