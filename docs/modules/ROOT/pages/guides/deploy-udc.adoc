...
networks because essential developer tools like *sncast* and *starkli* rely on this address by default when deploying contracts.
...
[TIP]
====
To avoid potential issues by using a different compiler version, you can directly declare the contract class deployed on Starknet mainnet and declare it on your appchain. Below *sncast* command shows how to declare the UDC class:

Quick reference:

```bash
sncast declare-from \
    --class-hash 0x01b2df6d8861670d4a8ca4670433b2418d78169c2947f46dc614e69f333745c8 \
    --source-network mainnet \
    --url <rpc-url>
```

For more details, check out the *sncast* guide about https://foundry-rs.github.io/starknet-foundry/starknet/declare.html#declaring-a-contract-by-fetching-it-from-a-different-starknet-instance[declaring a contract from a different Starknet instance].
