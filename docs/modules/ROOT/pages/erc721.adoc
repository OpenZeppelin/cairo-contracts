...
If `data` is not used, simply pass an empty `ByteArray`.
* `safe_transfer_from` is implemented such that the optional `data` argument is a `ByteArray` which is the Cairo equivalent to `bytes` in Solidity.
...
mod MyNFT {
    ...
    fn constructor(
        ...
        let token_id = 1;
        let token_uri: ByteArray = "NFT_URI";
