...
External interface for the extended `AccessControl` functionality.
...
.Events
--
* xref:IAccessControlWithDelay-RoleGrantedWithDelay[`++RoleAdminChanged(role, account, sender, delay)++`]
...
* xref:#AccessControlWithDelayImpl-get_role_status[`++get_role_status(self, role, account)++`]
* xref:#AccessControlWithDelayImpl-grant_role_with_delay[`++grant_role_with_delay(self, role, account, delay)++`]
