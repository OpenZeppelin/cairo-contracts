...
use openzeppelin_account::tests::mocks::eth_account_mocks::{
...
use openzeppelin_utils::test_utils::constants::ETH_PUBKEY;
...
fn setup_snake() -> (DualCaseEthAccount, EthAccountABIDispatcher) {
    ...
    let target = test_utils::deploy(SnakeEthAccountMock::TEST_CLASS_HASH, calldata);
    (
...
fn setup_camel() -> (DualCaseEthAccount, EthAccountABIDispatcher) {
    ...
    let target = test_utils::deploy(CamelEthAccountMock::TEST_CLASS_HASH, calldata);
    (
        DualCaseEthAccount { contract_address: target },
        EthAccountABIDispatcher { contract_address: target }
    )
...
fn setup_account_panic() -> (DualCaseEthAccount, DualCaseEthAccount) {
    let snake_target = test_utils::deploy(SnakeEthAccountPanicMock::TEST_CLASS_HASH, array![]);
    let camel_target = test_utils::deploy(CamelEthAccountPanicMock::TEST_CLASS_HASH, array![]);
    (
