:test-utils: https://github.com/OpenZeppelin/cairo-contracts/blob/main/tests/utils.py
...
Signature validation occurs separately from execution as of Cairo v0.10. Upon receiving transactions, an account contract first calls `\\__validate__`. An account will only execute a transaction if, and only if, the signature proves valid.
This decoupling allows for a protocol-level distinction between invalid and reverted transactions.

Account contracts must include two signature validation methods:

. `\\__validate__` - called to verify signature prior to transaction execution.
. `\\__validate_declare__` - called to verify signature prior to contract class declaration.
...
Initializes the Account contract with the passed public key.
...
Returns true if this contract implements the interface defined by `interfaceId`. Account contracts implement ERC165 through static support. In other words, Account interface support is hardcoded as opposed to ERC165 registration.
...
Validates the transaction signature. The StarkNet protocol calls the function prior to executing transactions.
The parameters must exactly match those passed to `\\__execute__`.
...
Validates the declared class hash.
...
----
namespace Account {
    func initializer(interfaceId: felt) -> (success: felt) {
    }
    ...
    func support_interface(interface_id: felt) -> (success: felt) {
    }
    ...
    func is_valid_eth_signature(hash: felt, signature_len: felt, signature: felt*) -> (is_valid: felt) {
    }
    ...
    func _execute_list(calls_len: felt, calls: Call*, response: felt*
    ) -> (response_len: felt) {
    }
...
Sets the public key for the Account contract. Should be called in the Account contract's constructor.
...
Guard function that reverts if not called by the contract itself.
