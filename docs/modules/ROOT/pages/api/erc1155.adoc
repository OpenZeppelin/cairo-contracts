...
:eip1155: https://eips.ethereum.org/EIPS/eip-1155#metadata[EIP1155]
...
Transfers `value` of `token_id` from `from` to `to`, checking first that `to` is able to handle ERC1155 tokens to
prevent getting them locked forever. For more information on how this is checked, see {receiving-tokens}.
...
Transfer `values` of `token_ids` (matched by index) from `from` to `to`, checking first that `to` is able to handle
ERC1155 tokens to prevent getting them locked forever.
For more information on how this is checked, see {receiving-tokens}.
...
https://eips.ethereum.org/EIPS/eip-1155#metadata[specified in the EIP].
...
the latter is not recognized as an account.
...
This function is called whenever an IERC1155 `token_id` token is transferred to this IERC1155Receiver implementor
