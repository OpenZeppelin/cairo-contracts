...
- ERC721_Enumerable tracks the total number of tokens by using the `all_tokens` mapping and `all_tokens_len` storage variable which together mimic an array in Cairo. The Solidity implementation instead uses an array
...
The current implementation of `safeTansferFrom` checks for `onERC721Received`. Because EOAs cannot be differentiated from regular contracts on StarkNet at this time, it's recommended to transfer tokens with `transferFrom`; however, please be aware that transfering tokens to a contract that does not support ERC721 can result in lost tokens forever. 
...
In order to ensure EVM/StarkNet compatibility, this ERC721 implementation does not calculate interface identifiers. Instead, the interface IDs are hardcoded from their Ethereum Virtual Machine (EVM) calculations. With the EVM, the interface ID is calculated from the selector's first four bytes of the hash of the function's signature; however, Cairo's protocol for calculating selectors includes 252 bytes. Due to this difference, hardcoding EVM's already-calculated interface IDs is the most consistent approach to both follow the EIP165 standard and EVM compatibility.
...
> EOA differentiation is still in the process of being resolved; therefore, this method should not be used until then.
