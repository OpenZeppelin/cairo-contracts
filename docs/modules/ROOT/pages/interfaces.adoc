...
Traits decorated with `#[starknet::interface]` automatically generate a dispatcher that can be used to interact with contracts that implement the given interface. They can be imported by appending the `Dispatcher` and `DispatcherTrait` suffixes to the trait name, like this:
...
WARNING: `camelCase` functions are deprecated and maintained only for Backwards Compatibility. We recommend to
integrate you contracts and components with the `snake_case` interfaces. The `camelCase` functions will be removed in
future versions.
