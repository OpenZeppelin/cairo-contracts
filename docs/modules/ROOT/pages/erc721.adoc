...
* `safe_transfer_from` is implemented such that the optional `data` argument is a span of felts.
...
Token URIs were previously stored as single field elements prior to Cairo v0.2.5; however, they are now stored as ``ByteArray``s.
