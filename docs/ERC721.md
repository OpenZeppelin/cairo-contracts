...
- `interface_id`s are hardcoded and initialized by the constructor. The hardcoded values derive from Solidity's selector calculations. Cairo calculates selectors differently; therefore, hardcoding the `interface_id`s already calculated with Solidity ensures EVM compatibility
...
- supporting additional `interface_id`s in ERC165's `supportsInterface` can be achieved using the setter method `ERC165_register_interface`. This follows OpenZeppelin's [ERC165Storage](https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/utils/introspection/ERC165Storage.sol) approach as opposed to the ERC165 standard. See [Supporting Interfaces](#supporting-interfaces)
- in line with the ERC165 interfaces design choice towards EVM compatibility, `IERC721_Receiver` compliant contracts (`ERC721_Holder`) return a hardcoded selector id according to EVM selectors, since selectors are calculated different in Cairo. ```
- the enumerable extension tracks the total number of tokens by using the `all_tokens` mapping and `all_tokens_len` storage variable. The Solidity implementation instead uses an array

## Usage

Use cases go from artwork, digital collectibles, physical property, and many more.

To show a standard use-case, we'll use the `ERC721_Mintable` preset which allows for only the owner to `mint` and `burn` tokens. Considering that the constructor method looks like this:
...
The ERC721_Mintable preset offers a quick and easy setup for creating NFTs. This preset includes two imported contracts: Ownable and ERC721_base. Further, this contract includes `mint` and `burn` methods to create and destroy NFTs. A common implementation of ERC721 contracts includes setting a check on the `mint` and `burn` methods to which only the owner can create and destroy tokens. In this preset, the owner is set in the constructor. 

### ERC721_Pausable

The ERC721_Pausable preset creates a contract with pausable token transfers, minting, and burning. This preset proves useful for scenarios such as preventing trades until the end of an evaluation period and having an emergency switch for freezing all token transfers in the event of a large bug. 
...
In order for the tokens to be correctly indexed, the frontend contract should also import the following superceded core methods:
-  `ERC721_Enumerable_transferFrom`
-  `ERC721_Enumerable_safeTransferFrom`
-  `ERC721_Enumerable_mint`
-  `ERC721_Enumerable_burn`

No additional changes are required in the constructor for deployment.
