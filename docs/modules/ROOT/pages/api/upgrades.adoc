...
[.contract]
[[IUpgradeable]]
=== `++IUpgradeable++` link:https://github.com/OpenZeppelin/cairo-contracts/blob/release-v0.16.0/packages/upgrades/src/interface.cairo[{github-icon},role=heading-link]
...
==== `[.contract-item-name]#++upgrade_and_call++#++(new_class_hash: ClassHash, selector: felt252, calldata: Span<felt252>)++` [.item-kind]#external#
...
=== `++UpgradeableComponent++` link:https://github.com/OpenZeppelin/cairo-contracts/blob/release-v0.16.0/packages/upgrades/src/upgradeable.cairo[{github-icon},role=heading-link]
...
==== `[.contract-item-name]#++upgrade_and_call++#++(ref self: ContractState, new_class_hash: ClassHash, selector: felt252, calldata: Span<felt252>)++` [.item-kind]#internal#
...
NOTE: The function call comes from the calling contract iself and not the account.

NOTE: This behavior can also be achieved with a list of calls from an account since the {snip6} account standard supports multicall.
...
The extra call would be executed from the account itself and can't be front-ran.
