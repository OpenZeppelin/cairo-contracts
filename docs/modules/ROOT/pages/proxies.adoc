...
The implementation contract should follow the xref:extensibility.md#the-pattern[Extensibility pattern] and import directly from the https://github.com/OpenZeppelin/cairo-contracts/blob/main/src/openzeppelin/upgrades/library.cairo[Proxy library].
...
As with most StarkNet contracts, interacting with a proxy contract requires an xref:eccount.md#quickstart[account abstraction].
