...
use openzeppelin_access::tests::mocks::ownable_mocks::{
    CamelOwnableMock, CamelOwnablePanicMock, SnakeOwnableMock, SnakeOwnablePanicMock
};
use openzeppelin_utils::serde::SerializedAppend;
use openzeppelin_utils::test_utils::constants::{OWNER, NEW_OWNER};
...
fn setup_snake() -> (DualCaseOwnable, IOwnableDispatcher) {
    ...
    let target = test_utils::deploy(SnakeOwnableMock::TEST_CLASS_HASH, calldata);
...
fn setup_camel() -> (DualCaseOwnable, IOwnableCamelOnlyDispatcher) {
    let mut calldata = array![];
    calldata.append_serde(OWNER());
    let target = test_utils::deploy(CamelOwnableMock::TEST_CLASS_HASH, calldata);
    (
...
fn setup_ownable_panic() -> (DualCaseOwnable, DualCaseOwnable) {
    let snake_target = test_utils::deploy(SnakeOwnablePanicMock::TEST_CLASS_HASH, array![]);
    let camel_target = test_utils::deploy(CamelOwnablePanicMock::TEST_CLASS_HASH, array![]);
    (
