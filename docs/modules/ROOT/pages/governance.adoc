...
The {cancel} function allows cancellers to cancel any `Waiting` operations.
...
Furthermore, the timelock component supports the concept of open roles which allows anyone to call a role-specific function.
To enable one of the roles to be open, grant the zero address with the target role.

WARNING: Be very careful with enabling open roles as _anyone_ can perform the role-specific task.
