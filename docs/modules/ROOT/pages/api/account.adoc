...
==== `[.contract-item-name]#++is_ValidSignature++#++(self: @ContractState, hash: felt252, signature: Array<felt252>) → felt252++` [.item-kind]#external#

See xref:ISRC6-is_valid_signature[ISRC6::is_valid_signature].

Supports the Cairo v0 convention of writing external methods in camelCase as mentioned in {dual-interfaces}.
