...
Copy the target preset contract from the {presets_dir} and paste it in the new project's `src/lib.cairo`.
...
[,bash]
----
starkli declare target/dev/my_project_<PRESET_CONTRACT>.contract_class.json \
  --network="sepolia"
----
