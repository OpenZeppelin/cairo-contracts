...
use openzeppelin_test_common::mocks::timelock::{
    BlockNumberTimelockControllerMock, IMockContractDispatcher, IMockContractDispatcherTrait,
...
use snforge_std::{
    CheatSpan, cheat_caller_address, start_cheat_block_number_global, start_cheat_caller_address,
    test_address,
};
...
type ComponentState =
    TimelockControllerComponent::ComponentState<BlockNumberTimelockControllerMock::ContractState>;

fn CONTRACT_STATE() -> @BlockNumberTimelockControllerMock::ContractState {
    @BlockNumberTimelockControllerMock::contract_state_for_testing()
}
...
fn deploy_timelock() -> TimelockABIDispatcher {
    start_cheat_block_number_global(0);
    let mut calldata = array![];
    ...
    let contract_address = utils::declare_and_deploy("BlockNumberTimelockControllerMock", calldata);
    TimelockABIDispatcher { contract_address }
}
...
fn deploy_attacker() -> ITimelockAttackerDispatcher {
    let calldata = array![];
    let contract_address = utils::declare_and_deploy("TimelockAttackerMock", calldata);
    ITimelockAttackerDispatcher { contract_address }
}
...
fn test_hash_operation() {
    let (timelock, target) = setup_dispatchers();
    let predecessor = 123;
    let salt = SALT;
...
fn test_hash_operation_and_hash_operations() {
    let (timelock, _) = setup_dispatchers();
    let predecessor = 123;
    let salt = SALT;
...
fn schedule_from_proposer(salt: felt252) {
    ...
    // Check timepoint
    let operation_timepoint = timelock.get_timepoint(target_id);
    let expected_timepoint = starknet::get_block_number() + delay;
    assert_eq!(operation_timepoint, expected_timepoint);
...
fn test_execute_when_scheduled() {
    ...
    // Fast-forward
    start_cheat_block_number_global(delay);
...
fn test_execute_early() {
    ...
    start_cheat_block_number_global(early_time);
...
fn test__before_call() {
    ...
    let target_time = MIN_DELAY + starknet::get_block_number();

    // Set targets in storage
    state.TimelockController_timestamps.write(target_id, target_time);

    // Fast-forward
    start_cheat_block_number_global(target_time);
...
fn test__before_call_insufficient_time() {
    ...
    // Fast-forward
    start_cheat_block_number_global(target_time - 1);
...
fn test__before_call_when_already_done() {
    ...
    // Fast-forward
    start_cheat_block_number_global(done_time);
...
fn test__after_call() {
    ...
    // Check timepoint is set to done (1)
    let done_timepoint = 1;
    let is_done = state.TimelockController_timestamps.read(target_id);
    assert_eq!(is_done, done_timepoint);
}
...
fn test__schedule() {
    ...
    let actual_timepoint = state.TimelockController_timestamps.read(target_id);
    let expected_timepoint = starknet::get_block_number() + delay;
    assert_eq!(actual_timepoint, expected_timepoint);
}
