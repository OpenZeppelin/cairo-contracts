...
:mint-api: xref:api/erc6909.adoc#ERC6909Component-mint[mint]
:burn-api: xref:api/erc6909.adoc#ERC6909Component-burn[burn]
...
Hooks are invoked by the internal `update` helper whenever tokens are transferred, minted, or burned:
...
Use the individual setters `set_token_name`, `set_token_symbol`, and `set_token_decimals` to configure
...
:update-token-supply-api: xref:/api/erc6909.adoc#ERC6909TokenSupplyComponent-update_token_supply[update_token_supply]
...
pub impl ERC6909HooksImpl of ERC6909Component::ERC6909HooksTrait<ContractState> {
    fn before_update(
        ...
        self
            ...
            .update_token_supply(from, recipient, id, amount);
    }
