...
Token URIs in Cairo are stored as single field elements which equates to 252-bits (or roughly 31 bytes). This means that a token's URI can be no longer than 31 characters. The `utils.py` module includes utility methods for converting to/from Cairo field elements. To properly interpret a URI from ERC721, simply trim the null bytes and decode the remaining bits as an ASCII string. For example:
...
felt = await erc721.tokenURI(first_token_id).call()
print(felt)
# 2219607741482918100109396223419758

string = felt_to_str(felt)
print(string)
# mock://mytoken
```
