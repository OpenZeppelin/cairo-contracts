...
The `VotesComponent` provides a flexible system for tracking voting power and delegation. This system allows users to delegate their voting power to other addresses, enabling more active participation in governance.
...
NOTE: When using this module, your contract must implement the {VotingUnitsTrait}. For convinience, this is done automatically for `ERC20` and `ERC721` tokens.
...
Special care must be taken to ensure that this function is called correctly. In `ERC20` and `ERC721` this is done usually via the hooks.
