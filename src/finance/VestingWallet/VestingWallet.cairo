...
mod VestingWalletcomponent{
    ...
    > of interface::IVestingWallet<ComponentState<TContractState>> {
        ...
        fn get_erc20_releasable(
            ...
            return self.vestedAmount(token, get_block_timestamp().into()) - self.erc20released.read(token);
        }
        ...
        fn release_erc20_token(ref self:  ComponentState<TContractState>, token: ContractAddress) -> bool {
            ...
            return IERC20Dispatcher { contract_address: token}.transfer(ownable_comp.Ownable_owner.read(), releasableAmount);
        }

        fn vestedAmount(self:  @ComponentState<TContractState>, token: ContractAddress, timestamp: u64) -> u256 {
    ...
    > of InternalTrait<TContractState> {
        ...
        fn _vestingSchedule(self: @ComponentState<TContractState>, totalAllocation: u256, timestamp: u64) -> u256  {
            if(timestamp < self.start.read()) {
                ...
                return totalAllocation * (timestamp - self.start.read().into()).into() / self.duration.read().into();
            }
        }
    }
