...
fn test_is_valid_p256_signature_good_sig() {
    ...
    let is_valid = is_valid_p256_signature(hash, data.public_key, serialized_good_signature.span());
    assert!(is_valid);
}
...
fn test_is_valid_p256_signature_invalid_format_sig() {
    ...
    is_valid_p256_signature(hash, data.public_key, serialized_bad_signature.span());
}
