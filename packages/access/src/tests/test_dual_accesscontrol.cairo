...
use openzeppelin_access::accesscontrol::interface::IAccessControlCamelDispatcher;
use openzeppelin_access::accesscontrol::interface::IAccessControlCamelDispatcherTrait;
...
use openzeppelin_access::tests::mocks::non_implementing_mock::NonImplementingMock;
...
fn setup_snake() -> (DualCaseAccessControl, IAccessControlDispatcher) {
    ...
    let target = test_utils::deploy(SnakeAccessControlMock::TEST_CLASS_HASH, calldata);
    (
...
fn setup_camel() -> (DualCaseAccessControl, IAccessControlCamelDispatcher) {
    let mut calldata = array![];
    calldata.append_serde(ADMIN());
    let target = test_utils::deploy(CamelAccessControlMock::TEST_CLASS_HASH, calldata);
    (
        DualCaseAccessControl { contract_address: target },
...
fn setup_non_accesscontrol() -> DualCaseAccessControl {
    let target = test_utils::deploy(NonImplementingMock::TEST_CLASS_HASH, array![]);
    DualCaseAccessControl { contract_address: target }
}

fn setup_accesscontrol_panic() -> (DualCaseAccessControl, DualCaseAccessControl) {
    let snake_target = test_utils::deploy(SnakeAccessControlPanicMock::TEST_CLASS_HASH, array![]);
    let camel_target = test_utils::deploy(CamelAccessControlPanicMock::TEST_CLASS_HASH, array![]);
    (
