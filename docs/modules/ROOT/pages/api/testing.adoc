...
Deploys an instance of a contract and unwraps the result. This function will skip declaration if the contract is already declared(the result of `snforge` `declare` call is of type `DeclareResult::AlreadyDeclared`).
...
Deploys an instance of a contract at a given address. This function will skip declaration if the contract is already declared(the result of `snforge` `declare` call is of type `DeclareResult::AlreadyDeclared`).
