...
:nonces-component: xref:api/utils.adoc#NoncesComponent[NoncesComponent]
:snip12-metadata: xref:api/utils.adoc#SNIP12Metadata[SNIP12Metadata]
...
When integrating the Votes component, the `VotingUnitsTrait` must be implemented to get the voting units for a given account as a function of the implementing contract. +
