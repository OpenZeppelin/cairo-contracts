...
- Collisions (e.g. mistakenly reusing the same storage slot from another component) are also possible, although less likely if best practices are followed, for example preppending storage variables with the component's name (e.g. `ERC20_balances`).
