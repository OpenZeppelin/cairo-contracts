...
use openzeppelin_testing::constants::{OWNER, OTHER_ADMIN, OTHER_ROLE, OTHER, ZERO, RECIPIENT};
...
fn setup_dispatcher() -> IERC2981ABIDispatcher {
    ...
    let contract_address = utils::declare_and_deploy("ERC2981AccessControlMock", calldata);
...
fn test_set_default_royalty_other_admin() {
    ...
    grant_accesscontrol_role(dispatcher, ROYALTY_ADMIN_ROLE, OTHER_ADMIN());
    start_cheat_caller_address(dispatcher.contract_address, OTHER_ADMIN());
    dispatcher.set_default_royalty(RECIPIENT(), FEE_NUMERATOR);
...
fn test_set_default_royalty_invalid_role() {
    ...
    grant_accesscontrol_role(dispatcher, OTHER_ROLE, OTHER_ADMIN());
    start_cheat_caller_address(dispatcher.contract_address, OTHER_ADMIN());
    dispatcher.set_default_royalty(RECIPIENT(), FEE_NUMERATOR);
}
...
fn grant_accesscontrol_role(
    dispatcher: IERC2981ABIDispatcher, role: felt252, admin: ContractAddress
) {
    start_cheat_caller_address(dispatcher.contract_address, OWNER());
    AccessControlABIDispatcher { contract_address: dispatcher.contract_address }
