...
a safe transfer. This function must return the `IERC1155Receiver` interface id
...
Called whenever the implementing contract receives `values` through
a safe batch transfer. This function must return the `IERC1155Receiver` interface id
to confirm the token transfer.
